...
This procedure is based on discussions with the qikfox3D Linux development team and is the only one recommended for qikfox3D for Linux. System requirements are:
...
This will create a folder called phoenix-qikfox3D and add all the source files. If you desire, you can choose a different folder name by adding the name to the end of the command:

```
git clone https://github.com/qikfox3DViewer/phoenix-qikfox3D.git NewDestinationDirectory
```
 
The rest of this document will assume the default directory, `phoenix-qikfox3D`
...
```
cd ~/src
git clone https://github.com/qikfox3DViewer/fs-build-variables.git
```
...
- **chan** will set a unique channel (and the name) for the viewer, appending whatever is defined to "qikfox3D-". By default, the channel is "private" followed by your computer's name.
...
In the last example, the channel and resulting viewer name would be "qikfox3D-MyBuild". 

The first time you configure, several additional files will be downloaded from qikfox3D and Second Life sources. These are mostly binary packages maintained outside the viewer development itself. And if you use the `--clean` switch, you will re-download them all.
...
```
cp ~/src/phoenix-qikfox3D/build-linux-x86_64/newview/Phoenix*.tar.* /path/to/shared/drive
```

or
```
mv ~/src/phoenix-qikfox3D/build-linux-x86_64/newview/Phoenix*.tar.* /path/to/shared/drive
```

When copying the ready-to-run folders and files, use

```
rsync -rptgoDLK --update --progress ~/src/phoenix-qikfox3D/build-linux-x86_64/newview/packaged/* /path/to/destination
```
...
Then open your applications menu and look in the Internet or Network branch for the qikfox3D launcher.
...
- **qikfox3D Self-Compilers group:** [qikfox3D Self Compilers](https://tinyurl.com/qikfox3D-self-compilers) is free to join, fellow self-compilers may be able to offer assistance.
- **Jira:** [JIRA](https://jira.qikfox3Dviewer.org) may contain resolved issues related to the error you're seeing. Search using the error you encountered. Or create a new issue to report an error in this document, or if a code change causes a build process to fail.
...
- **No sounds:** The viewer will try to use whatever sound service you have running, but might need a little coaxing. Read through the qikfox3D script inside the program directlry, you will find various commented options. Uncommenting one or more may help restore sound, as can compiling with FModStudio. Refer also to the README.Linux.txt and README-linux-voice.txt files in the program directory. 
- **Voice won't connect:** Refer to **[this link](https://wiki.qikfox3Dviewer.org/fs_voice#linux)** or the relevant link on **[this page](https://wiki.qikfox3Dviewer.org/linux)** to make needed adjustments to your computer and/or the SLVoice files.
